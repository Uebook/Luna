...
class OrderController extends AdminBaseController
{
    ...
    public function status($id1, $status)
    {
        ...
        if (($status === 'completed' || $status === 'delivered') && 
            ...
            try {
                // Calculate amount in BHD (if currency_value exists, divide by it; otherwise use pay_amount directly)
                $amountBHD = $order->pay_amount;
                if ($order->currency_value && $order->currency_value > 0) {
                    $amountBHD = $order->pay_amount * $order->currency_value;
                }
                
                // Award points: 1 BHD = 1 point
                if ($amountBHD > 0 && $order->user_id) {
                    // Check if WalletController exists in API namespace
                    if (class_exists('\App\Http\Controllers\Api\WalletController')) {
                        \App\Http\Controllers\Api\WalletController::awardPointsOnPurchase($order->user_id, $amountBHD, $order->id);
                    }
                }
            } catch (\Exception $e) {
