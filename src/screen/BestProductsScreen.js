...
// - Product grid with discount badges and ratings
...
// API helper function to fetch best products
const fetchBestProducts = async () => {
    try {
        console.log('Fetching best products...');

        const response = await fetch(`${BASE_URL}/screen/products/best`, {
            method: 'Post',
            headers: {
        ...
        console.log('Raw best products response:', responseText.substring(0, 200));
        ...
        console.log('Parsed best products data:', data);
...
export default function BestProductsScreen(props = {}) {
    ...
    // Fetch best products on component mount
    useEffect(() => {
        loadData();
    }, []);
    ...
    const loadData = async () => {
        try {
            ...
            Alert.alert(
                'Error',
                'Failed to load best products. Please check your connection and try again.'
            );
            setData([]);
        } finally {
    ...
    const Banner = () => (
        <View style={styles.bannerContainer}>
            ...
            >
                <Text style={styles.bannerTitle}>Best Products</Text>
                <Text style={styles.bannerSubtitle}>Handpicked quality items just for you</Text>
            </LinearGradient>
        </View>
    );
    ...
    const EmptyList = () => (
        <View style={styles.emptyWrap}>
            <Icon name="trophy-outline" size={48} color={THEME.gray} />
            <Text style={styles.emptyTitle}>
                {isSearching ? 'No products found' : 'No best products available'}
            </Text>
            <Text style={styles.emptySub}>
                {isSearching ? 'Try a different search term.' : 'Check back later for featured products.'}
            </Text>
            {!isSearching && (
...
const createStyles = (THEME, TEXT_DARK) => StyleSheet.create({
    ...
    card: {
        backgroundColor: THEME.white, borderRadius: 16, padding: 10, marginVertical: 8,
        borderWidth: 1, borderColor: '#eef2ff', position: 'relative'
    },
    cardDisabled: {
        opacity: 0.6,
    },
    ...
    sheet: {
        backgroundColor: THEME.white, paddingTop: 8, paddingHorizontal: 16, paddingBottom: 24,
