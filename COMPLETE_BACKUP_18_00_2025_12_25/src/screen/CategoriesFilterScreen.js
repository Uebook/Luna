import React, { useState, useEffect } from 'react';
import {
    View,
    Text,
    StyleSheet,
    SafeAreaView,
    ScrollView,
    TouchableOpacity
} from 'react-native';
import Icon from 'react-native-vector-icons/Feather';
import MaterialIcons from 'react-native-vector-icons/MaterialIcons';
import { useSkeletonLoader } from '../hooks/useSkeletonLoader';
import { SkeletonCategoryScreen } from '../components/SkeletonLoader';
import StandardHeader from '../components/StandardHeader';

const categoriesData = [
    {
        name: 'Clothing',
        image: '👗',
        expanded: true,
        subcategories: [
            'Dresses', 'Pants', 'Skirts', 'Shorts',
            'Jackets', 'Hoodies', 'Shirts', 'Polo',
            'T-Shirts', 'Tunics'
        ]
    },
    {
        name: 'Shoes',
        image: '👟',
        expanded: false,
        subcategories: ['Sneakers', 'Boots', 'Heels', 'Sandals']
    },
    {
        name: 'Bags',
        image: '👜',
        expanded: false,
        subcategories: ['Handbags', 'Backpacks', 'Crossbody', 'Wallets']
    },
    {
        name: 'Lingerie',
        image: '🩲',
        expanded: false,
        subcategories: ['Bras', 'Panties', 'Sleepwear', 'Shapewear']
    },
    {
        name: 'Accessories',
        image: '🧢',
        expanded: false,
        subcategories: ['Belts', 'Hats', 'Jewelry', 'Scarves']
    },
    {
        name: 'Just for You',
        image: '✨',
        special: true,
        subcategories: []
    }
];

const CategoriesFilterScreen = ({ navigation }) => {
    const [loading, setLoading] = useSkeletonLoader(true, 600);
    const [gender, setGender] = useState('Female');
    const [categories, setCategories] = useState(categoriesData);
    const [selectedSubs, setSelectedSubs] = useState([]);

    useEffect(() => {
        const timer = setTimeout(() => setLoading(false), 800);
        return () => clearTimeout(timer);
    }, [setLoading]);

    const toggleExpand = (index) => {
        const updated = [...categories];
        updated[index].expanded = !updated[index].expanded;
        setCategories(updated);
    };

    const toggleSelect = (name) => {
        setSelectedSubs((prev) =>
            prev.includes(name) ? prev.filter((s) => s !== name) : [...prev, name]
        );
    };

    const renderSubcategory = (name, index) => {
        const selected = selectedSubs.includes(name);
        return (
            <TouchableOpacity
                key={index}
                style={[styles.subcategoryButton, selected && styles.subcategorySelected]}
                onPress={() => toggleSelect(name)}
            >
                <Text style={[styles.subcategoryText, selected && styles.subcategoryTextActive]}>
                    {name}
                </Text>
            </TouchableOpacity>
        );
    };

    const handleSearch = () => {
        navigation.navigate('CategoryProductScreen', {
            gender,
            subcategories: selectedSubs,
        });
    };

    if (loading) {
        return (
            <SafeAreaView style={styles.container}>
                <StandardHeader
                    title="Categories Filter"
                    navigation={navigation}
                    showGradient={true}
                />
                <SkeletonCategoryScreen />
            </SafeAreaView>
        );
    }

    return (
        <SafeAreaView style={styles.container}>
            <ScrollView contentContainerStyle={styles.content}>
                {/* Header */}
                <View style={styles.header}>
                    <Text style={styles.headerTitle}>All Categories</Text>
                    <TouchableOpacity onPress={() => navigation.goBack()}>
                        <Icon name="x" size={24} color="#000" />
                    </TouchableOpacity>
                </View>

                {/* Gender Filter */}
                <View style={styles.genderRow}>
                    {['All', 'Female', 'Male'].map((g) => (
                        <TouchableOpacity
                            key={g}
                            style={[styles.genderBtn, gender === g && styles.genderSelected]}
                            onPress={() => setGender(g)}
                        >
                            <Text style={[styles.genderText, gender === g && styles.genderTextActive]}>
                                {g}
                            </Text>
                        </TouchableOpacity>
                    ))}
                </View>

                {/* Category Sections */}
                {categories.map((cat, index) => (
                    <View key={cat.name} style={styles.categorySection}>
                        <TouchableOpacity
                            style={styles.categoryHeader}
                            onPress={() => (cat.special ? null : toggleExpand(index))}
                        >
                            <View style={styles.catTitleWrap}>
                                <Text style={styles.catIcon}>{cat.image}</Text>
                                <Text style={styles.catTitle}>{cat.name}</Text>
                            </View>
                            {cat.special ? (
                                <MaterialIcons name="arrow-forward" size={20} color="#000" />
                            ) : (
                                <Icon
                                    name={cat.expanded ? 'chevron-up' : 'chevron-down'}
                                    size={20}
                                    color="#000"
                                />
                            )}
                        </TouchableOpacity>

                        {cat.expanded && cat.subcategories.length > 0 && (
                            <View style={styles.subcategoriesGrid}>
                                {cat.subcategories.map(renderSubcategory)}
                            </View>
                        )}
                    </View>
                ))}

                {/* Search Button */}
                <TouchableOpacity
                    style={styles.searchBtn}
                    onPress={handleSearch}
                    disabled={selectedSubs.length === 0 && gender === 'All'}
                >
                    <Text style={styles.searchBtnText}>Search</Text>
                </TouchableOpacity>
            </ScrollView>
        </SafeAreaView>
    );
};

const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#fff' },
    content: { padding: 20, paddingBottom: 40 },

    header: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
        marginBottom: 20,
    },
    headerTitle: { fontSize: 22, fontWeight: 'bold', color: '#000' },

    genderRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 },
    genderBtn: {
        flex: 1,
        paddingVertical: 10,
        marginHorizontal: 4,
        backgroundColor: '#F2F2F2',
        borderRadius: 10,
        alignItems: 'center',
    },
    genderSelected: { backgroundColor: '#004BFE' },
    genderText: { fontWeight: '600', color: '#000' },
    genderTextActive: { color: '#fff' },

    categorySection: { marginBottom: 14 },
    categoryHeader: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
        backgroundColor: '#F8F8F8',
        padding: 12,
        borderRadius: 10,
    },
    catTitleWrap: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    catIcon: { fontSize: 20 },
    catTitle: { fontWeight: 'bold', fontSize: 15 },

    subcategoriesGrid: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 10, gap: 10 },
    subcategoryButton: {
        paddingVertical: 10,
        paddingHorizontal: 14,
        borderWidth: 1,
        borderColor: '#FFD9D9',
        borderRadius: 10,
    },
    subcategorySelected: {
        backgroundColor: '#004BFE',
        borderColor: '#004BFE',
    },
    subcategoryText: { fontSize: 14, color: '#000' },
    subcategoryTextActive: { color: '#fff' },

    searchBtn: {
        marginTop: 20,
        backgroundColor: '#004BFE',
        paddingVertical: 14,
        borderRadius: 12,
        alignItems: 'center',
    },
    searchBtnText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
});

export default CategoriesFilterScreen;
