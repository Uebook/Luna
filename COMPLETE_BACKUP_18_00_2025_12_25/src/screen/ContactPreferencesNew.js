...
const Section = ({ title, right, children, style, styles }) => (
    <View style={[styles.section, style]}>
        <Row style={{ justifyContent: 'space-between', marginBottom: 6 }} styles={styles}>
...
const LinkBtn = ({ label, onPress, styles, theme }) => (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}>
        <Text style={[styles.link, { color: theme.p1 }]}>{label}</Text>
    </TouchableOpacity>
);

const Chip = ({ icon, label, styles, theme, isDark }) => (
    <View style={styles.chip}>
        <Icon name={icon} size={14} color={theme.p1} />
        <Text style={styles.chipTxt}>{label}</Text>
    </View>
);

const ToggleRow = ({ icon, title, subtitle, value, onChange, styles, theme, isDark }) => (
    <Row style={styles.toggleRow} styles={styles}>
        <Row style={{ gap: 12, flex: 1 }} styles={styles}>
            <View style={styles.bulletIcon}>
                <Icon name={icon} size={16} color={theme.p1} />
            </View>
            <View style={{ flex: 1 }}>
        ...
        <Switch
            ...
            thumbColor={Platform.OS === 'android' ? (value ? '#fff' : (isDark ? theme.line : '#f4f3f4')) : undefined}
            trackColor={{ false: isDark ? theme.line : '#D8E1F5', true: theme.p1 }}
            ios_backgroundColor={isDark ? theme.line : '#D8E1F5'}
        />
    </Row>
);

/* ---------- Reusable identity editor modal ---------- */
const IdentityEditModal = ({ visible, type, initialValue, onClose, onSave, styles, theme, isDark }) => {
        ...
        <Modal animationType="slide" transparent visible={visible} onRequestClose={onClose}>
            ...
            >
                ...
                <View style={styles.modalCard}>
                    <Row style={{ justifyContent: 'space-between', marginBottom: 8 }} styles={styles}>
                        <Text style={styles.modalTitle}>{title}</Text>
                        <TouchableOpacity onPress={onClose} accessibilityLabel={t('a11y.close')}>
                            <Icon name="x" size={20} color={theme.sub} />
                        </TouchableOpacity>
                    </Row>
                    ...
                    <View style={[styles.inputWrap, !!error && { borderColor: theme.red || '#EF4444' }]}>
                        <TextInput
                            placeholder={
                                type === 'email' ? t('placeholders.email') : t('placeholders.phoneExample')
                            }
                            placeholderTextColor={isDark ? theme.sub : '#9AA6B2'}
                    ...
                    <Row style={{ gap: 10, marginTop: 14 }} styles={styles}>
                        <TouchableOpacity style={[styles.btn, styles.btnSecondary]} onPress={onClose}>
                            <Text style={[styles.btnTxt, { color: theme.p1 }]}>{t('common.cancel')}</Text>
                        </TouchableOpacity>
    ...
    return (
        <SafeAreaView style={styles.safe}>
            <View style={styles.container}>
                {/* Standard Header */}
                <StandardHeader 
                    title={t('titles.contactPreferences')}
                ...
                >
                    ...
                    <View style={[styles.hero, { padding: s(16) }]}>
                        <Row style={{ justifyContent: 'space-between', marginBottom: s(6) }} styles={styles}>
                            <Text style={[styles.heroTitle, { fontSize: s(15) }]}>{t('hero.title')}</Text>
                            <Icon name="settings" size={s(18)} color={isDark ? theme.p4 : '#E9F2FF'} />
                        </Row>
                        <Text style={[styles.heroSub, { fontSize: s(12), lineHeight: s(18) }]}>
                            {t('hero.sub')}
                        </Text>

                        <Row style={{ flexWrap: 'wrap', gap: s(8), marginTop: s(10) }} styles={styles}>
                            {activeChips.length ? (
                                activeChips.map((c, idx) => <Chip key={idx} icon={c.icon} label={c.label} styles={styles} theme={theme} isDark={isDark} />)
                            ) : (
                    ...
                    {/* Email */}
                    <Section title={t('sections.email.title')} styles={styles}>
                        <Row style={{ justifyContent: 'space-between', alignItems: 'center' }} styles={styles}>
                            <Text style={styles.identity}>{t('sections.email.identity')}</Text>
                            <LinkBtn label={t('common.change')} onPress={() => openModal('email')} styles={styles} theme={theme} />
                        </Row>
                    ...
                    {/* SMS */}
                    <Section title={t('sections.sms.title')} styles={styles}>
                        <Row style={{ justifyContent: 'space-between', alignItems: 'center' }} styles={styles}>
                            <Text style={styles.identity}>{t('sections.sms.identity')}</Text>
                            <LinkBtn label={t('common.change')} onPress={() => openModal('sms')} styles={styles} theme={theme} />
                        </Row>
                    ...
                    {/* WhatsApp */}
                    <Section title={t('sections.wa.title')} styles={styles}>
                        <Row style={{ justifyContent: 'space-between', alignItems: 'center' }} styles={styles}>
                            <Text style={styles.identity}>{t('sections.wa.identity')}</Text>
                            <LinkBtn label={t('common.change')} onPress={() => openModal('whatsapp')} styles={styles} theme={theme} />
                        </Row>
                    ...
                    <View style={styles.legalBox}>
                        <Icon name="shield" size={16} color={theme.p1} />
                        <Text style={styles.legalTxt}>
                            {t('legal.prefix')}{' '}
                            <Text style={[styles.link, { color: theme.p1 }]} onPress={openPolicy}>{t('legal.privacy')}</Text>. {t('legal.help')}{' '}
                            <Text style={[styles.link, { color: theme.p1 }]} onPress={openHelp}>{t('legal.here')}</Text>.
                        </Text>
                    </View>
                </ScrollView>
                ...
                <View style={[styles.saveWrap, { padding: s(16) }]}>
                    <TouchableOpacity
                        ...
                        style={[
                            styles.saveBtn,
                            { height: s(48), backgroundColor: theme.p1 },
                            !somethingChanged && { opacity: 0.6 },
                        ]}
                    >
                        <Row style={{ justifyContent: 'center', gap: s(8) }} styles={styles}>
...
const createStyles = (theme, isDark) => StyleSheet.create({
    ...
    hero: { borderRadius: 16, backgroundColor: theme.p2 },
    heroTitle: { color: isDark ? theme.p4 : '#E9F2FF', fontWeight: '700' },
    heroSub: { color: isDark ? theme.p4 : '#E9F2FF', opacity: 0.95 },
    ...
    badge: {
        fontSize: 12,
        color: theme.p1,
        backgroundColor: isDark ? theme.p4 : '#E7EEFF',
        paddingHorizontal: 8,
        paddingVertical: 4,
        borderRadius: 999,
        overflow: 'hidden',
    ...
    bulletIcon: {
        ...
        height: 26,
        borderRadius: 13,
        backgroundColor: isDark ? theme.p4 : '#E9F0FF',
        alignItems: 'center',
        justifyContent: 'center',
    },
    tTitle: { fontSize: 13.5, color: theme.text, fontWeight: '600' },
    tSub: { fontSize: 12, color: theme.sub, marginTop: 2, lineHeight: 16 },

    identity: { fontSize: 12, color: theme.sub, textAlign: I18nManager.isRTL ? 'right' : 'left' },
    identityValue: { fontSize: 13, color: theme.text, marginTop: 4, marginBottom: 8, textAlign: I18nManager.isRTL ? 'right' : 'left' },

    link: { color: theme.p1, fontWeight: '700' },

    chip: {
        flexDirection: 'row',
        alignItems: 'center',
        backgroundColor: isDark ? theme.p4 : '#DDE8FF',
        paddingHorizontal: 10,
        paddingVertical: 6,
        borderRadius: 999,
        gap: 6,
    },
    chipTxt: { fontSize: 12, color: theme.p1, fontWeight: '700' },
    ...
    legalBox: {
        ...
        padding: 12,
        borderRadius: 12,
        backgroundColor: isDark ? theme.card : '#EEF4FF',
        borderWidth: 1,
        borderColor: theme.line,
        marginTop: 16,
    },
    legalTxt: { flex: 1, color: theme.text, fontSize: 12, lineHeight: 17, textAlign: I18nManager.isRTL ? 'right' : 'left' },

    saveWrap: {
        position: 'absolute',
        left: 0, right: 0, bottom: 0,
        backgroundColor: isDark ? theme.bg + 'F0' : 'rgba(247,250,255,0.92)',
    },
    saveBtn: {
        borderRadius: 14,
        justifyContent: 'center',
    ...
    modalBackdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.35)' },
    modalCard: {
        backgroundColor: theme.card,
        padding: 16,
    ...
    modalTitle: { fontSize: 16, fontWeight: '700', color: theme.text, textAlign: I18nManager.isRTL ? 'right' : 'left' },
    modalHint: { fontSize: 12, color: theme.sub, marginTop: 6, marginBottom: 10, textAlign: I18nManager.isRTL ? 'right' : 'left' },
    inputWrap: {
        borderWidth: 1,
        borderColor: theme.line,
        borderRadius: 12,
    ...
    errorTxt: { color: theme.red || '#EF4444', marginTop: 6, fontSize: 12, textAlign: I18nManager.isRTL ? 'right' : 'left' },
    ...
    btnSecondary: { backgroundColor: isDark ? theme.line : '#F3F6FF' },
    btnPrimary: { backgroundColor: theme.p1, borderColor: theme.p1 },
