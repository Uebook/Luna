...
const ProductDetailScreen = () => {
    ...
    // Reviews and Related Products State
        ...
        const fetchProductDetails = async () => {
            ...
            try {
                ...
                if (data.status) {
                    ...
                    // Set reviews/ratings from API
                    if (Array.isArray(data.ratings)) {
                        const formattedReviews = data.ratings.map((rating) => ({
                            id: rating.id,
                            user: rating.user_name || 'Anonymous',
                            rating: rating.rating || 0,
                            comment: rating.review || '',
                            date: rating.review_date ? new Date(rating.review_date).toLocaleDateString() : 'Recently'
                        }));
    ...
    // Fetch popular products and related products
    useEffect(() => {
        const fetchRelatedProducts = async () => {
            ...
            try {
                ...
                let formattedPopular = [];
                let formattedRelated = [];
                
                // Fetch popular/hot products
                try {
                    const popularResponse = await fetch('https://luna-api.proteinbros.in/public/api/v1/screen/products/hot', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Accept': 'application/json',
                    ...
                    console.log('🔥 Popular products API response:', popularData);
                    
                    if (popularData.status && Array.isArray(popularData.products)) {
                        formattedPopular = popularData.products
                            .filter(p => p.id != product.id) // Exclude current product (use != for loose comparison)
                            .slice(0, 10) // Limit to 10 items
                            .map(p => {
                                // Build image URL - use photo field from API
                                let imageUrl = 'https://via.placeholder.com/150x150';
                                if (p.photo) {
                                    if (p.photo.startsWith('http://') || p.photo.startsWith('https://')) {
                                        imageUrl = p.photo;
                                    } else {
                                        imageUrl = `https://proteinbros.in/assets/images/products/${p.photo}`;
                                    }
                                } else if (p.image) {
                                    if (p.image.startsWith('http://') || p.image.startsWith('https://')) {
                                        imageUrl = p.image;
                                    } else {
                                        imageUrl = `https://proteinbros.in/assets/images/products/${p.image}`;
                                    }
                                }
                        ...
                        console.log('🔥 Formatted popular products:', formattedPopular);
                ...
                // Use hot products from same category if category_id exists, otherwise use all hot products
                if (product.category_id) {
                    // We already have popular products filtered by category, so use them as related
                    if (formattedPopular.length > 0) {
                        formattedRelated = formattedPopular.slice(0, 8);
                    } else {
                        // Fallback: fetch hot products without category filter
                        try {
                            const relatedResponse = await fetch('https://luna-api.proteinbros.in/public/api/v1/screen/products/hot', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                            ...
                            if (relatedData.status && Array.isArray(relatedData.products)) {
                                formattedRelated = relatedData.products
                                ...
                                console.log('🔗 Formatted related products:', formattedRelated);
                            }
                        } catch (error) {
                ...
                // Set related products - use related if available, otherwise use popular as fallback
                if (formattedRelated.length > 0) {
                    setRelatedProducts(formattedRelated);
                } else if (formattedPopular.length > 0) {
                    setRelatedProducts(formattedPopular);
                } else {
                    setRelatedProducts([]);
                }
            } catch (error) {
    ...
    }, [product?.id, product?.category_id, userId]);
    ...
    const renderPopularProduct = ({ item }) => (
        ...
        >
            ...
            <Text style={styles.popularProductPrice}>{formatPrice(item.price || 0)}</Text>
        </TouchableOpacity>
    );
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.ratingSummary}>
                        <Text style={styles.rating}>{averageRating > 0 ? `${averageRating}/5` : 'No rating'}</Text>
