...
export default function GiftCardSuccess({ route, navigation }) {
    ...
    const shareIt = async () => {
        try {
            const shareMessage = `🎁 Luna Gift Card!\n\n` +
                `Recipient: ${form?.name || 'N/A'}\n` +
                `Amount: ${amountText}\n` +
                `Gift Card Code: ${giftCardCode || 'N/A'}\n` +
                (expiresAt ? `Expires: ${new Date(expiresAt).toLocaleDateString()}\n` : '') +
                `\nRedeem this gift card in the Luna app wallet section!`;
            
            await Share.share({
                message: shareMessage,
                title: 'Luna Gift Card',
                url: selectedCard?.image, // Some platforms may use this as image
            });
        } catch (error) {
                ...
                {giftCardCode && (
                    <View style={styles.codeContainer}>
                        <Text style={[styles.codeLabel, isRTL && { textAlign: 'right' }]}>
                            {t('codeLabel', { defaultValue: 'Gift Card Code' })}
                        </Text>
                        <View style={[styles.codeBox, isRTL && { flexDirection: 'row-reverse' }]}>
                            <Text style={styles.codeText}>{giftCardCode}</Text>
                            <TouchableOpacity
                                onPress={() => {
                                    Clipboard.setString(giftCardCode);
