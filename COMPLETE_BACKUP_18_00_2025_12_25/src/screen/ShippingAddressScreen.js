...
export default function ShippingAddressScreen({ navigation }) {
    ...
    const COLORS = {
        ...
        p1: theme.p1,
        p2: theme.p2,
        p3: theme.p3 || theme.p1,
        p4: theme.p4 || theme.brandSoft,
        red: theme.red,
        white: theme.white,
        muted: theme.sub,
        gray: theme.gray || theme.sub,
        ink: theme.ink || theme.text,
        brandSoft: theme.brandSoft || theme.p4,
        gradients: {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // Fetch from API
                const response = await api.post('/address/list', { user_id: userId });
                if (response.data.success && response.data.addresses) {
                    setAddressesState(response.data.addresses);
                } else {
    ...
    const handleDelete = async (id) => {
        try {
            const userId = await AsyncStorage.getItem('user_id') || '1';
            await api.post(`/address/delete/${id}`, { user_id: userId });
            // Reload addresses
            const response = await api.post('/address/list', { user_id: userId });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (editingId) {
                // Update via API
                await api.post(`/address/update/${editingId}`, {
                    user_id: userId,
                    name: form.fullName,
                    phone: form.phone,
    ...
    const handleQuickPrimary = async (id) => {
        try {
            const userId = await AsyncStorage.getItem('user_id') || '1';
            await api.post(`/address/set-default/${id}`, { user_id: userId });
            // Reload addresses
            const response = await api.post('/address/list', { user_id: userId });
            if (response.data.success) {
                setAddressesState(response.data.addresses || []);
